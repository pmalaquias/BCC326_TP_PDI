...
class ImprovedAttentionDetector:
            ...
            if os.path.exists(metadata_path_auto):
                try:
                    with open(metadata_path_auto, 'r') as f:
                        metadata = json.load(f)
                    print(
                        f"  📋 Metadados encontrados automaticamente: {metadata_path_auto}")
                    return metadata
                except Exception as e:
    ...
    def extract_enhanced_face_region(self, frame, face):
        ...
        gray_face = cv2.cvtColor(face_region, cv2.COLOR_BGR2GRAY) if len(
            face_region.shape) == 3 else face_region
        eyes = self.eye_cascade.detectMultiScale(
            gray_face, scaleFactor=1.1, minNeighbors=5, minSize=(10, 10))
    ...
    def detect_attention_enhanced(self, frame):
        ...
        face_region, coords, quality = self.extract_enhanced_face_region(
            frame, face)
...
def main():
    ...
    if detector.metadata:
        ...
        print(
            f"  • Threshold do treinamento: {detector.metadata.get('attention_threshold', 'N/A')}")
        print(
            f"  • Épocas treinadas: {detector.metadata.get('training_epochs', 'N/A')}")
        print(
            f"  • Accuracy final: {detector.metadata.get('final_val_accuracy', 'N/A')}")
        print(
            f"  • Total de sequências: {detector.metadata.get('total_sequences', 'N/A')}")
    ...
    try:
        while True:
            ...
            try:
                cv2.imshow(
                    'Detector de Atenção - Treinamento Compatível', display_frame)
                    ...
                    if not calibration_mode:
                        ...
                        if len(calibration_predictions) > 10:
                            ...
                            new_threshold = np.percentile(
                                predictions_array, 25)  # 25º percentil

                            print(f"\n📊 CALIBRAÇÃO CONCLUÍDA:")
                            print(
                                f"  • Amostras coletadas: {len(calibration_predictions)}")
                            print(
                                f"  • Threshold anterior: {detector.attention_threshold:.3f}")
                            print(
                                f"  • Threshold sugerido: {new_threshold:.3f}")

                            # Aplicar novo threshold
                            detector.attention_threshold = new_threshold
                            print(
                                f"  ✅ Novo threshold aplicado: {new_threshold:.3f}")
                        else:
                            print(
        ...
        print(
            f"📊 Qualidade facial média: {final_stats['avg_face_quality']:.2f}")
